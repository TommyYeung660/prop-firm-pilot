...
| **Close verification** | `CloseControlPlane` 的 modify readback 已具備 retry-with-delay（default 0.5s + 1 retry），避免 broker propagation lag 導致的 false mismatch | v1.5.0_stable 04-08 patch: CADJPY 6+ 小時 verify_failed 問題已解決 |
| **Tactical exit timing** | `_should_defensive_exit_initial_risk()` 已加入 `defensive_exit_min_hold_seconds` guard（production=300s），避免 5m 噪音在開倉後數分鐘內觸發 IRSF | v1.5.0_stable 04-08 patch: 4/6 trades premature IRSF exit 問題已解決 |
| **Per-symbol tuning** | `SymbolTacticalOverride` + `TacticalConfig.symbol_overrides` 可 per-symbol 覆蓋 `defensive_exit_loss_r`、`defensive_exit_min_hold_seconds` | v1.5.0_stable 04-08 patch: EURCHF 低波動對特化配置已啟用 |
| **Close attribution** | `CloseReconciler` 會驗證 PnL sign vs close reason 一致性，避免負 PnL 記為 `tp_hit` | v1.5.0_stable 04-01 patch: GBPJPY PnL sign contradiction 已修復 |
| **ATR resilience** | `TacticalValidator` 在 1H bars 不可用時自動切換到 5m ATR fallback（×√12 scale） | v1.5.0_stable 04-01 patch: `insufficient_1h_data` 永久 SKIP 問題已解決 |
| **Portfolio pre-check** | `PortfolioRiskGuard` 在 tactical validation 前做 portfolio risk 預檢（fail-open） | v1.5.0_stable 04-01 patch: 已知會被拒的交易不再白跑 pipeline |
...
| 工作流 | 目前狀態 | 需要的交付 |
|---|---|---|
| **Open-book worst-case risk guard** | `PortfolioRiskGuard` pre-check 已落地（v1.5.0_stable 04-01 patch），但 aggregate worst-case natural-SL 組合風險仍未完整約束 | portfolio-level open-risk reservation / aggregate worst-case guard |
...
- v1.5.0_stable 04-01 patch 修復了 close attribution、ATR resilience、portfolio pre-check
- v1.5.0_stable 04-08 patch 修復了 IRSF exit timing、modify readback、per-symbol tuning
...
- ✅ v1.5.0_stable 04-01 patch: close attribution PnL sign、ATR 5m fallback、portfolio pre-check
- ✅ v1.5.0_stable 04-08 patch: IRSF exit timing guard、modify readback retry、per-symbol tactical overrides
...
- stable runtime 已通過兩輪 production evidence 驗證（v1.5.0_stable 04-01 patch + v1.5.0_stable 04-08 patch）
...
> `prop-firm-pilot` 已通過 `v1.5.0_stable 04-01 patch` + `v1.5.0_stable 04-08 patch` 兩輪 production-driven 修復，runtime 穩定性與 exit 精準度顯著提升；接下來的重點是 portfolio / memory / exposure closure，而不是架構變更。
